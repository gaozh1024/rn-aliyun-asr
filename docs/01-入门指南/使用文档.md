...
Android 无需在主项目中手动添加 AAR 依赖。框架会在构建时自动从 `nuisdk-release.aar` 解压 `classes.jar` 与 JNI 库，并完成编译与运行时集成。
...
| 模式 | 说明 |
|------|------|
| `VadMode.MODE_P2T` | 按住说话模式（官方推荐默认模式） |
| `VadMode.MODE_VAD` | 自动检测语音开始和结束 |
